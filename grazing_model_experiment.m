...
function out = grazing_model_experiment(meta)
	...
	% declare arrays
	tab      = table();
	% coefficient of variation of exogenous heterogeneity of carrying capacity
	cv_k = [0.01   0.02      0.05      0.1000    0.2000    0.3000    0.4000    0.5000    0.6,      0.7,      0.8,      0.9,      1.0];
	tab.cv_k(1:length(cv_k)) = cv_k;
	% mean biomass of initial pattern
	...
	for adx=1:nsa
	    ...
	    [stat,rad] = analyze(mu0_a(adx),k*tab.cv_k(adx));

	    % tabulate properties of model run
	    tab.runtime(adx) = sum(stat.runtime);
	    tab.filter_order(adx) = stat.filter_order;
	    tab.p_coverage(adx) = stat.p_coverage;
	    tab.fl(adx) = stat.fl;
	    tab.r2_bt(adx) = stat.r2_bt;
	    tab.r2_Sr(adx) = stat.r2_Sr;
	    tab.p_disagreement(adx) = stat.p_disagreement;
	    tab.coherence(adx) = stat.coherence;
	    tab.mu_b(adx) = stat.mu_b;

	    % pattern, unscaled
	    splitfigure([3,ceil(nsa/3)],[1,adx],fflag,'',1000);
	    cla();
	    imagesc(stat.x,stat.x,stat.b);
	    xlabel('$x$','interpreter','latex');
	    ylabel('$y$','interpreter','latex');
	    clim(quantile(stat.b(:),[0.1,0.9]));
	    axis square;
	    colormap(fcmap(256));
	    ...
	    cla();
	    imagesc(stat.x*stat.fl_lp,stat.x*stat.fl_lp,stat.b);
	    ...
	    xh = logspace(log10(quantile(stat.b(:),0.001)),log10(quantile(stat.b(:),0.999)),nx);
	    %histogram(p.b(:),xh,'Normalization','pdf','DisplayStyle','stairs');
	    histogram(log(stat.b(:)),log(xh),'Normalization','pdf','DisplayStyle','stairs');
	    %vline(g);
	    ...
	    vline(log(b0));
	    vline(log(stat.g),'linestyle','-','color','b');
	    %log(xhc(1))
	    %pause
	    ...
	    cla();
	    imagesc(fftshift(stat.fx)/stat.fl_lp,fftshift(stat.fy)/stat.fl_lp,fftshift(stat.hatS)*stat.fl_lp^2);
	    s = 4;
	    xlim([-1,1]*s);
	    ylim([-1,1]*s);
	    S2d = trifilt2(fftshift(stat.hatS),3);
	    caxis([0,0.5*max(S2d*stat.fl_lp^2,[],'all')]);
	    axis square
	    ...
	    imagesc(fftshift(x)*stat.fl_lp,fftshift(x)*stat.fl_lp,fftshift(stat.Rhat));
	    s = 1.0;
	    xlim([-1,1]*s);
	    ylim([-1,1]*s);
	    axis square
	    ...
	    yyaxis left
	    cla
	    plot(stat.fr/stat.fl_lp,stat.fl_lp*stat.Sr.normalized,'.','linewidth',1);
	    hold on;
	    plot(stat.fr/stat.fl_lp,stat.fl_lp*stat.Srlp,'-','linewidth',1);
	    s=4.0;
	    xlim([0,s]);
	    ...
	    yyaxis right
	    cla;
	    plot(stat.fr/stat.fl_lp,stat.S.coherence.radial,'linewidth',1);
	    ylim([0,1]);
	    ...
	    cla();
	    plot(stat.theta,stat.St,'linewidth',1)
	    hold on;
	    ...
	    cla();
	    imagesc(stat.x,stat.x,stat.b_overlay);
	    axis(Lp*[0,1,0,1]);
	    axis equal;
	    xlabel('$x$','interpreter','latex');
	    ylabel('$y$','interpreter','latex');
	    ...
	    cla();
	    imagesc(stat.x*stat.fl_lp,stat.x*stat.fl_lp,stat.b_overlay);
	    axis equal;
	    xlim([0,10]);
	    ylim([0,10]);
	    xlabel('Position $x/\lambda_l$','interpreter','latex');
	    ...
	    yyaxis left
	    cla();
	    plot(stat.t,[stat.mu,stat.sd]);
	    yyaxis right
	    cla();
	    plot(mid(stat.t),stat.d);
	    xlabel('time')
	    legend('$\bar b$','std(b)','1/||b||db/dt','dbarb/dt','||b(t)-b(T)||/||b(T)||','interpreter','latex');
	    end % for idx
	end % for adx
	...
	function val = fobj(mu_b0,sd_k)
		prec = analyze(mu_b0,sd_k);
		val = stat.p_coverage;
	end

	function [stat,rad] = analyze(mu_b0,sd_k)
		disp([mu_b0,sd_k]);
		...
		param.opt.heterogeneity_oversampling_factor_spectral = 3;
		...
		if (exist(f_str,'file'))
			...
			stat.mu_b = mean(b,'all');
			stat.sd_b = std(b,[],'all');
			stat.me_b = median(b,'all');
			...
			stat.filter_fc = par(1);
			stat.filter_order = par(2);
			...
			stat.b = b;
			stat.fx = fx;
			stat.b_overlay = b_overlay;
			stat.b_thresh = b_thresh;
			stat.blp_thresh = blp_thresh;
			stat.sd = sd;
			stat.mu = mu;
			stat.Sr = Sr;
			stat.Srlp = Srlp;
			...
			stat.theta = theta;
			stat.St = St;
			stat.p_disagreement = mean(b_overlay == 1,'all') + mean(b_overlay == 2,'all');
			stat.g = g;

			stat.r2_b = corr(flat(blp),flat(b));
			% corr_a(adx,2) = corr(flat(Sr.normalized),flat(Srlp))
			%stat.corr(2) = wcorr(cvec(fr),sqrt(flat(Sr.normalized)),cvec(fr),sqrt(flat(Srlp)));
			Sr.normalized(1) = 0;
			c = corr(flat(blp_thresh),flat(b_thresh));
			stat.r2_bt = sign_to_pearson(c.^2);

			stat.r2_Sr = fitstat.goodness.r2;
			%df = 1./param.L(1);
			% hellinger_distance(flat(Sr.normalized),flat(Srlp),df,fr);
			stat.coherence = sp.stat.coherence.radial;
			stat.S.coherence.radial = sp.S.coherence.radial;

			save(f_str,'stat','sp');
		end % if not yet analyzed
