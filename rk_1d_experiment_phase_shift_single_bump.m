...
function [x,bb,w,tp,rk] = rk_1d_phase_shift_experiment_single_bump(meta)
	...
	% if larger, left transition influences right transition
	wwin   = 3;
	...
	for ldx = 1:length(L0)
	    for adx = 1:length(da)
	        ...
	        % ouput time step
	        param.opt.dto = 200;
	        % standard deviation of bare soil infiltration coefficient
	        param.pss.a  = 0;
	        param.pmu.ex = ex;
	        param.pmu.vx = vx;
	        param.initial_condition = 'obj.random_state()'; %random';
	        param.opt.path_str = 'mat/rk-experiment-phase-shift-single-bump/';
	        ...
	        rk = Rietkerk(param);
	        if (isempty(fc0))
	        	disp('Running without perturbation to determine characeristic wavelength');
	        	[t,y]   = rk.run();
	        	figure(1e3);
	        	subplot(2,2,1);
	        	cla
	        	y0 = rk.initial_condition_from_central_frequency(y(end,:));
	        	plot(rk.x,[y(end,1:end/3)',y0(1:end/3)]);
	        	drawnow
	        else
	        	y0 = rk.initial_condition_periodic(fc);
	        end
	        param.initial_condition = y0;
	        % To speed up this process, restart model with initially sinusoidal wave
	        disp('Running second time without perturation, sinudsoidal intial state');
	        rk = Rietkerk(param);
	        % run model
	        [t,y] = rk.run();
	        y = double(y);
	        ...
	        T = 1e4;
	        x0 = [0.1,0.6]*L;
	        % two mutually compensating humps
	        param.pmu.a = a.*(1 + ...
	        	(ap(adx)-1)*( ...
	        			- 1.0*exp(-0.5*(cvec((rk.x-x0(1)))/L0).^2) ...
	        			+ 1.0*exp(-0.5*(cvec((rk.x-x0(2)))/L0).^2) ...
	            )); 
	        param.pss.a = 0;
	        param.T =  T;
	        %param.opt.dt = dt;
	        param.initial_condition = y0;
	        rk = Rietkerk(param);
	        disp('Running with perturbation');
	        [t,y]   = rk.run();
	        y       = single(y);
	        [b,w,h] = rk.extract1(y(end,:)');
	        ...
	        plot((rk.x-x0(2))/lambda_c,rk.p.a/a_bar,'linewidth',linewidth);
	        ...
	        yyaxis left
	        cla
	        plot((rk.x-x0(2))/lambda_c,Lc/lambda_c,'linewidth',linewidth)
	        xlim((xl-x0(2))/lambda_c);
	        xlabel('Distance $x/\bar\lambda_c$','interpreter','latex')
	        ylim([0,1.05]);
	        hold on
	        plot((rk.x-x0(2))/lambda_c,b_c/b_c_bar,'-r','linewidth',linewidth);
	        xlim((xl-x0(2))/lambda_c);
	        ylim([0,1.05]);
	        ...
	        plot((rk.x-x0(2))/lambda_c,b_c/b_c_bar,'linewidth',linewidth);
	        xlim((xl-x0(2))/lambda_c);
	        xlabel('Distance $x/\bar\lambda_c$','interpreter','latex');
	        ...
	        yyaxis right
	        plot((rk.x-x0(2))/lambda_c,rk.p.a/a_bar,'linewidth',1,'linewidth',linewidth);
	        xlim((xl-x0(2))/lambda_c);
	        ylim([0.75,1.15]);
